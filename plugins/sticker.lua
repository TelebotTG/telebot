...
do
  ...
  function run(msg, matches)
    ...
    if is_chat_msg(msg) and not msg.media then
      ...
      -- create a group
      if matches[1] == 'mkgroup' and matches[2] and is_momod(msg.from.id, msg.to.id) then
      ...
      if data[tostring(msg.to.id)] then
        ...
        if matches[1] == 'setabout' and matches[2] and is_momod(msg.from.id, msg.to.id) then
          data[tostring(msg.to.id)]['description'] = matches[2]
          save_data(_config.moderation.data, data)
        return 'Set group description to:\n'..matches[2]
        elseif matches[1] == 'about' then
          return get_description(msg, data)
        elseif matches[1] == 'setrules' and is_sudo(msg.from.id, msg.to.id) then
        ...
        elseif matches[1] == 'group' then
          ...
          if matches[2] == 'lock' then
            ...
            elseif matches[3] == 'name' and is_momod(msg.from.id, msg.to.id) then
            ...
            elseif matches[3] == 'member' and is_momod(msg.from.id, msg.to.id) then
            ...
            elseif matches[3] == 'photo' and is_momod(msg.from.id, msg.to.id) then
        ...
        elseif matches[1] == 'sticker' then
          if matches[2] == 'warn' then
            ...
            return 'تم تفعيل حظر الملصقات.\n'
                   ..'سوف يتم تنبيه اولا ثم الحظر بلانتهاك الثاني.'
        ...
        -- set group name
        elseif matches[1] == 'setname' and is_momod(msg.from.id, msg.to.id) then
          settings.set_name = string.gsub(matches[2], '_', ' ')
          save_data(_config.moderation.data, data)
          rename_chat(get_receiver(msg), settings.set_name, ok_cb, false)
        -- set group photo
        elseif matches[1] == 'setphoto' and is_momod(msg.from.id, msg.to.id) then
          settings.set_photo = 'waiting'
          save_data(_config.moderation.data, data)
